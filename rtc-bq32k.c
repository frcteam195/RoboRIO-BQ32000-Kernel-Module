 ...
 * Copyleft  (L) 2017 Marshall Massengill <marshall@team900.org>
 *
 * Marshall's changes are to avoid calling this in some complicated way 
 * and instead just force the trickle charge enable by moving the function
 *
...
/* Nothing calls this anymore because #Marshall
                                ...
                                const struct i2c_device_id *id)
{
        ...
        */
        ...
        if (error)
                return error;
        regbat = 0x20;
        error = bq32k_write(dev, &regbat, BQ32K_TCH2, 1);
        if (error)
                return error;
        dev_info(dev, "Enabled trickle RTC battery charge.\n");
        /* End Marshall's awesome modifications */

        /* Remove call to old function
        if (client && client->dev.of_node) {
                trickle_charger_of_init(dev, client->dev.of_node);
                dev_info(dev, "Marshall Test 3 - Enabled trickle RTC battery charge.\n");
        }
        */
